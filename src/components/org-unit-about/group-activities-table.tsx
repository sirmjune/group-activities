...
export function GroupActivitiesTable(props: Props) {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const orgUnitCodeResponse = await fetch(
                    `${process.env.REACT_APP_DHIS2_BASE_URL}/ovc/api/organisationUnits/${props.orgUnitId}`,
                ...
                if (orgUnitCode) {
                    ...
                    const codeResponse = await fetch(
                        `${process.env.REACT_APP_DHIS2_BASE_URL}/ovc/api/trackedEntityAttributes/oqabsHE0ZUI/generate?ORG_UNIT_CODE=${orgUnitCode}`,
    ...
    const fetchNewId = async () => {
        try {
            const response = await fetch(
                `${process.env.REACT_APP_DHIS2_BASE_URL}/ovc/api/system/id?`,
    ...
    async function handleFormSubmit(event: React.FormEvent) {
        ...
        try {
            const response = await fetch(
                `${process.env.REACT_APP_DHIS2_BASE_URL}/ovc/api/events?`,
    ...
    const handleBeneficiaryIdBlur = async () => {
        ...
        try {
            const response = await fetch(
                `${process.env.REACT_APP_DHIS2_BASE_URL}/ovc/api/trackedEntityInstances/query.json?ouMode=ACCESSIBLE&program=RDEklSXCD4C&attribute=HLKc2AKR9jW:EQ:${enteredValue}&paging=false`,
